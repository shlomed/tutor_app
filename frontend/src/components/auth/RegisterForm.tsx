...
interface Props {
  onRegister: (username: string, name: string, password: string) => Promise<void>
  onSuccess: () => void
}

export function RegisterForm({ onRegister, onSuccess }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <label className="block text-sm font-semibold text-navy-700 mb-1.5">{UI.fullName}</label>
        <input
          type="text"
          value={name}
